...
- Core steering files: !`bash -lc 'set -Eeuo pipefail; IFS=$"\n\t"; shopt -u histexpand; ls -la "{{KIRO_DIR}}"/steering/*.md 2>/dev/null || echo "No steering directory found"'`
- Custom steering count: !`bash -lc 'set -Eeuo pipefail; IFS=$"\n\t"; shopt -u histexpand; if [ -d "{{KIRO_DIR}}/steering" ]; then count=0; for f in "{{KIRO_DIR}}"/steering/*.md; do if [ -f "$f" ] && [ "$f" != "{{KIRO_DIR}}/steering/product.md" ] && [ "$f" != "{{KIRO_DIR}}/steering/tech.md" ] && [ "$f" != "{{KIRO_DIR}}/steering/structure.md" ]; then count=$((count + 1)); fi; done; echo "$count"; else echo "0"; fi'`

### Project Analysis
- Specialized areas: !`bash -lc 'set -Eeuo pipefail; IFS=$"\n\t"; shopt -u histexpand; find . -path "./node_modules" -prune -o -path "./.git" -prune -o -type d \( -name "test*" -o -name "spec*" -o -name "api" -o -name "auth" -o -name "security" \) -print 2>/dev/null || echo "No specialized directories found"'`
- Config patterns: !`bash -lc 'set -Eeuo pipefail; IFS=$"\n\t"; shopt -u histexpand; find . -path "./node_modules" -prune -o \( -name "*.config.*" -o -name "*rc.*" -o -name ".*rc" \) -print 2>/dev/null || echo "No config files found"'`
