...
- Custom steering count: !`if [ -d "{{KIRO_DIR}}/steering" ]; then count=0; for f in {{KIRO_DIR}}/steering/*.md; do if [ -f "$f" ] && [ "$f" != "{{KIRO_DIR}}/steering/product.md" ] && [ "$f" != "{{KIRO_DIR}}/steering/tech.md" ] && [ "$f" != "{{KIRO_DIR}}/steering/structure.md" ]; then count=$((count + 1)); fi; done; echo "$count"; else echo "0"; fi`
