...
# ${description}
# Generated by amazonq-sdd v0.1.3 - Compatible with ${info.command} (${info.type})
...
if [ $? -eq 0 ]; then
    ...
    # Provide guidance on saving output for spec commands
    if [[ "${commandName}" == *"spec"* ]] && [ -n "$FEATURE_NAME" ]; then
        echo ""
        echo "📝 IMPORTANT: Save Amazon Q's response to:"
        case "${commandName}" in
            *"init"*)
                echo "   $FEATURE_DIR/specification.md"
                ;;
            *"requirements"*)
        ...
        echo ""
        echo "💡 TIP: Use 'nano $FEATURE_DIR/[filename].md' or copy-paste the content above"
    fi
else
